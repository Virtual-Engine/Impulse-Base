...
namespace WardrobeMenuVars {
	...
	void SaveCurrentOutfit(String fileName) {
		...
		j["outfit"]["model"] = ENTITY::GET_ENTITY_MODEL(GetLocalPlayer().m_ped);
...
void WardrobeMenu::Init() {
	...
	//	.addSubmenu<WardrobePresetsMenu>().addHotkey()
	//	.addTooltip("Preset outfits"));

	//addOption(SubmenuOption("Load outfits")
	//	.addSubmenu<LoadWardrobeMenu>()
	...
	addOption(ButtonOption("Save outfit")
		.addFunctions([] {
			GetFiberPool()->DoJob([] {
				...
				MISC::DisplayOnscreenKeyboard(true, (char*)"Name Outfit:", (char*)"", (char*)"", (char*)"", (char*)"", (char*)"", 32);
				while (MISC::UpdateOnscreenKeyboard() == 0) {
					GetFiberManager()->GoToMainFiber();
				}
				if (!MISC::GetOnscreenKeyboardResult()) {

				}
				SaveCurrentOutfit(MISC::GetOnscreenKeyboardResult());
				pMenu.bOpen = false;
				});
			})
	...
	addOption(ButtonOption("Police Uniform")
		.addFunction([] {
			if (ENTITY::GET_ENTITY_MODEL(GetLocalPlayer().m_ped) == Utils::GetHashKey("mp_f_freemode_01")) {
				//hat
	...
	for (size_t i = 0; i < 12; i++) {
		...
		addOption(NumberOption<int>(SCROLL, apparanceComponentLabel[i])
			...
			.addTooltip(apparanceComponentLabel[i]));
	}
}
