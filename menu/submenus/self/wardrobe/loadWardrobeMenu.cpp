...
void LoadWardrobeMenu::Init() {
	...
	addOutfitsOption = addOption(ButtonOption("~c~Add outfits to ./Documents/Pegasus/Outfits")
		.addTooltip("Add outfits to ./Documents/Pegasus/Outfits"));
}
		...
		if (!m_outfitFiles.empty()) {
			for (string& outfit : m_outfitFiles) {
				addOption(ButtonOption(outfit.c_str())
					//.hasNoTranslation()
					.addFunctions([&] { LoadOutfit(outfit.c_str()); })
